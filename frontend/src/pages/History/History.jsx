...
export default function History() {
  ...
  const handleViewRecord = (record, type) => {
    ...
    else if (type === 'RFP' || type === 'RFP') path = '/rfp';
  ...
  const handleViewLogReference = (log) => {
    ...
    else if (log.resource === 'RFP' || log.resource === 'RFP') record = rfps.find(r => r.id === log.resourceId);
        ...
        {activeTab === 'rfps' && (
          <TicketTable 
            tickets={rfps
              ...
              .map(r => ({ ...r, requestorName: r.requestor || r.author?.name || (r.rfpNo ? `RFP #${r.rfpNo}` : 'Unnamed RFP') }))} 
