...
export default function PendingRecords() {
  ...
  const fetchData = async () => {
    try {
      ...
      const allPending = [
        ...parsedTickets.filter(t => t.status === 'Pending' || t.status === 'Pending Endorsement' || t.status === 'Pending Approval' || t.status === 'Endorsed' || !t.status).map(t => ({ 
            ...t, 
            docType: 'TRIP_TICKET',
            displayType: 'TT',
        ...
        ...parsedPrfs.filter(p => p.status === 'Pending' || p.status === 'Pending Verification' || p.status === 'Pending Approval' || p.status === 'Verified' || !p.status).map(p => ({
            ...p,
            docType: 'PRF',
            displayType: 'PRF',
            apiEndpoint: '/prfs',
        ...
        ...parsedRfps.filter(r => r.status === 'Pending' || r.status === 'Pending Dept Head Approval' || r.status === 'Pending Final Approval' || (r.status === 'Approved' && !r.receivedBy) || !r.status).map(r => ({
            ...r,
            docType: 'RFP',
            displayType: 'RFP',
            apiEndpoint: '/rfps',
      ...
      const filteredPending = allPending.filter(record => {
        ...
        if (record.docType === 'PRF') {
            ...
            if (record.status === 'Pending Approval' || record.status === 'Verified') {
