  ...
  return (
    <div className="trip-ticket-print-wrapper print-only" style={{ 
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #000', paddingBottom: '15px', marginBottom: '22px' }}>
        <img 
          src={(ticket?.company || ticket?.author?.company || user?.company?.name) === 'Adventures' ? "/Adventures_Logo.png" : (ticket?.company || ticket?.author?.company || user?.company?.name) === 'Capital' ? "/CGI_Logo.png" : "/HDI Primary Logo .png"} 
          alt="Company Logo" 
          style={{ height: '75px' }} 
        />
