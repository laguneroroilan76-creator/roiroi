...
export default function ArchivedRecords() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const rfps = Array.isArray(rfpsRes.data) ? rfpsRes.data : [];
      ...
      const allArchived = [
        ...
        ...rfps.filter(r => ['Archived', 'Disapproved', 'Cancelled'].includes(r.status)).map(r => ({ ...r, type: 'RFP', displayType: 'RFP' }))
      ];
  ...
  const handleView = (record) => {
    ...
    else if (record.type === 'RFP' || record.type === 'RFP') path = '/rfp';
